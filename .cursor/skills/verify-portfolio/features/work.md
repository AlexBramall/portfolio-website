...
- `work-home-grid` shows three case cards under `Selected work`.
...
- **Home grid.** Load home. Run `control-portfolio browser goto` then `control-portfolio browser wait-for --selector "#selected-work"`. Three cards are visible. There are no `Previous projects` / `Next projects` controls.
