...
- **Desktop Resume.** Choose `Resume`. Run `control-portfolio browser click --role link --name "Resume"` then `control-portfolio browser wait-for --selector "#resume"`. Heading `Resume` is in view and the path ends with `/resume`. The header `Resume` link has `aria-current="page"` and the active pill. It is not a filled primary button and it does not leave the app.
