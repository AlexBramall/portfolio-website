...
- **Footer strip.** Run `control-portfolio browser eval --expr "document.getElementById('about-cta').scrollIntoView({block:'start'})"` then `control-portfolio browser wait-for --selector "#about-cta"`. The strip repeats `Contact` and `Resume`.
